...
def train_model(train_texts, train_labels, eval_texts, eval_labels):
    ...
    eval_dataset = TextDataset(eval_texts, eval_labels)  # 추가된 부분
    training_args = TrainingArguments(
        ...
        logging_dir='./logs',
        logging_steps=10,
        evaluation_strategy="epoch"  # Deprecated 메시지에 따라 변경
    )
    trainer = Trainer(
        model=model,
        args=training_args,
        train_dataset=train_dataset,
        eval_dataset=eval_dataset,  # 추가된 부분
...
results['core-set'].append(initial_results['eval_f1'])
...
# 액티브 러닝을 통한 반복 학습
for strategy in ['random', 'entropy', 'core-set', 'hybrid']:
    initial_texts = start_data['content'].tolist()
    initial_labels = start_data['score'].tolist()
    print(strategy)
    for i in range(5):
        ...
        indices = active_learning_sample(data_pool, strategy, 25, trainer)
...
train_sizes = [1000, 1250, 1500, 1750, 2000]

for strategy in results.keys():
    plt.plot(train_sizes, results[strategy], label=strategy)
...
plt.grid(True)
plt.savefig(link+'/active_learning_performance.png')
plt.show()
